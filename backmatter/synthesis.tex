...
\section{Back to the slide on Maya's desk}

The Foreword put a slide deck in front of Maya. The question on the last slide was a version of \textit{do we go?} The pages between that question and this one are the long answer.
...
\begin{itemize}
\item \textbf{Alignment.} \textit{What is this agent supposed to do, in writing, and who agrees with that paragraph?} If the answer is fewer than three names from outside her team, she sends the deck back.
\item \textbf{Hardening.} \textit{What is the worst thing this agent can do if every input arrives malicious?} If the answer is hand-wavy, she sends the deck back.
\item \textbf{Benchmarking.} \textit{What does \textbf{better than the alternative} mean for this agent, and on whose number?} If the vendor is the only source, she sends the deck back.
\item \textbf{Evaluation.} \textit{What is on the dashboard the day after launch, and who looks at it the day after that?} If the dashboard is a screenshot, she sends the deck back.
\item \textbf{Certification.} \textit{Whose signature ships this, and what is the trigger that brings it back to that signature?} If there is no named signer and no named trigger, she sends the deck back.
\end{itemize}

Maya does not need every answer to be polished. She needs every answer to be \textit{specific}, \textit{owned}, and \textit{defensible}. If the five answers are all of those things, she signs. If they are not, she does not. The decision was always hers; what changed in the weekend she spent with this book is that she now knows what she is signing.
