...
\begin{TNdefinition}{Drift}
The slow change in an agent's behavior over time. Three flavors: \textit{model drift} (the model itself was updated), \textit{data drift} (the world the agent sees has changed), and \textit{concept drift} (what counts as a correct answer has changed). All three are inevitable; only continuous evaluation catches them in time to act.
\end{TNdefinition}
