...
\subsection*{Sizing the queue}

The previous warning has a numerical companion. If you cannot work out how many humans the escalation queue will need, you have not designed the queue --- you have written a wish.

The arithmetic is unforgiving but simple. Take the agent's projected daily volume, multiply by the escalation rate, and you have the queue's daily inflow. A customer-service agent doing 10{,}000 turns a day at a 2\% escalation rate puts \textbf{200 items} into a human queue every day. A clinical triage assistant doing 1{,}000 cases a day at a 10\% escalation rate puts 100 items in --- but each one is heavier.

Reviewer throughput is domain-bound. The high-volume floor, set by content-moderation operations at scale, is around 200 simple decisions per reviewer per day (the publicly reported Facebook ratio is roughly 3 million flagged items against 15{,}000 moderators \citep{mit-tr-fb-moderators-2020}). For a regulated-domain agent --- claims adjudication, clinical review, financial-advice escalation --- the realistic ceiling is 40 to 80 cases per reviewer per day, because each case carries reading, a chart note, and a sign-off. Tonneau et al.'s HateDay study \citep{tonneau-hateday-2024} finds that even with best public classifiers, at least 10\% of daily content needs human review to meet the policy threshold regulators set --- a reminder that automation alone does not bring the queue to zero.

A working formula:

\begin{quote}
Reviewers $=$ (daily volume $\times$ escalation rate) $/$ (cases per reviewer per day) $\times$ shift-coverage multiplier $\times$ bench factor.
\end{quote}

The shift-coverage multiplier is the part teams forget. A queue that operates 24/7 needs at least eight reviewers on a single site or six per site across two time zones, by the same on-call math the SRE book applies to engineering rotations \citep{google-sre-book} --- not because eight people are doing the work, but because fewer than eight cannot cover nights, weekends, vacation, and sickness. The bench factor is the additional 20 to 30\% you carry for training, leave, and the inevitable attrition in a job most people do not love.

Plug in the numbers before the launch meeting. If the answer is ``we will figure it out,'' the answer is the warning above.

\begin{TNnew}
\subsection*{When the queue is staffed and still fails}

The arithmetic above sizes the queue under the assumption that a reviewer who reads a case will catch the agent's error. The harder, evidenced reality is that humans approve because the agent is usually right. On high-volume, low-base-rate queues the rational reviewer move is to click through, and the oversight pillar gets satisfied on paper while delivering no real control.

Three human-factors phenomena predict this:

\begin{itemize}
\item \textbf{Automation bias} --- the operator over-relies on the system's recommendation and stops searching for disconfirming evidence \citep{parasuraman-manzey-2010}.
\item \textbf{Vigilance decrement} --- attention to rare signals decays sharply after the first 20--30 minutes of a shift \citep{warm-parasuraman-matthews-2008}.
\item \textbf{Reviewer deskilling} --- over months the reviewer can no longer reproduce the agent's judgement unaided, and so cannot challenge it \citep{bainbridge-1983}.
\end{itemize}

Four design moves push back against these. Treat them as practices, not options.

\begin{itemize}
\item \textbf{Forced justification.} Reviewers pick from a structured reason taxonomy --- or type a minimum-length note --- before approve and deny are enabled. The click-through path is closed.
\item \textbf{Blind re-review.} A defined sample (5\% is a common floor) is shown to a second reviewer with the agent's recommendation hidden. Inter-rater disagreement is tracked monthly; spikes recalibrate the gate.
\item \textbf{Planted-error injection.} Synthetic incorrect agent outputs are salted into the live queue at a known rate of 1--3\%. \textit{Catch rate}, not throughput, is the queue's real KPI. A falling catch rate is the early signal of deskilling.
\item \textbf{Rotation.} No reviewer stays on a single queue beyond a defined cap; cross-train across at least two queues so judgement does not narrow to one workflow's pattern.
\end{itemize}

Without these, the headcount math sizes the queue and nothing else.
\end{TNnew}
...
The failure shape most multi-agent deployments hit first is not collusion or emergent loops. It is the quieter one: Agent A hallucinates a fact, Agent B accepts it as a premise, Agent C takes an action based on B's reasoning, and the audit trail at every hop looks locally coherent. The UC Berkeley MAST study \citep{cemri2025mast} ran more than 200 tasks through seven popular orchestration frameworks and catalogued fourteen distinct failure modes; the largest single category was inter-agent misalignment, and within it the dominant pattern was a downstream agent treating an upstream agent's output as ground truth without any provenance check.

Two mechanisms make this worse than the single-agent case. The first is \textbf{compounding hallucination}: each hop adds a small probability of fabrication, and the joint probability of an end-to-end clean trajectory drops geometrically. A 5\% per-hop hallucination rate becomes a 23\% chance of a contaminated five-hop trajectory. The second is \textbf{conformity bias}: once one agent commits to a confident assertion, downstream agents preferentially align with it rather than challenge it, even when their own evidence contradicts. This is the multi-agent analogue of human anchoring, and it is observable in production traces.

Three mitigations are doing real work in 2026 deployments. \textbf{Per-hop validation} --- each agent restates the upstream claim in its own words and the orchestrator checks for drift before allowing the next call. \textbf{Provenance tags on every assertion} --- outputs carry a structured \texttt{source: \{tool | retrieval | inference | prior-agent\}} field, and any agent acting on a chain that traces back to \texttt{inference} from another agent requires explicit elevation. \textbf{Cross-hop drift detection} --- sample trajectories offline, label the upstream claim, and measure how often the downstream agent's interpretation diverges from it; anything over a few percent is a tuning signal, not noise. None of this is free. All of it is cheaper than the first incident where Agent C wired money to a counterparty because Agent A misread an email.
...
\item Build human-in-the-loop with the downstream queue, the owner, and an attention-design plan (forced justification, blind re-review, planted-error injection, rotation) attached; an escalation that becomes a rubber stamp is worse than no escalation.
...
\item For each approval queue, salt the inflow with planted errors at a known rate; report catch rate monthly and track inter-rater disagreement on a blind-review sample.
