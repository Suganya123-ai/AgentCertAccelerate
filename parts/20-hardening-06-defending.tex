...
\begin{TNnew}
\subsection*{The A2A boundary: identity, authority, provenance, liability}

Google's \textit{A2A} is an emerging 2025 protocol for structured communication between agents from different vendors \citep{a2a-spec}. Like MCP for tools, it standardizes the handshake. The protocol is the easy part. The four things it does not give you --- and that Maya's team owns --- are the things the boundary actually needs.

\textbf{Delegated authority.} When agent~$A$ calls agent~$B$ on user~$U$'s behalf, what scope, audience, and TTL is being passed? The model that holds up under audit is the \textit{capability token}: \verb|action| $\times$ \verb|resource| $\times$ \verb|audience| $\times$ \verb|expiry|, downscoped at each hop rather than impersonated. OAuth~2.0 Token Exchange (RFC~8693) is the closest standard primitive \citep{rfc8693}. A2A's \textit{agent cards} describe what an agent \textit{can} do; they do not say what it has been \textit{authorized} to do for this caller. That authorization layer is the deployer's responsibility.

\textbf{Federated identity.} Each agent must prove who it is across organizational boundaries. The workable building blocks today are workload identity (SPIFFE / SPIRE \citep{spiffe-spec}), mTLS with pinned roots, and signed agent cards. The longer-horizon direction is W3C Decentralized Identifiers and Verifiable Credentials \citep{w3c-did-core,w3c-vc-data-model}, where an agent presents a credential that survives the transport layer. ``Trust the cloud the call came from'' is not the answer.

\textbf{Verifiable provenance.} Every A2A call must carry a signed chain back to the originating user and any intermediate agents. Months later, you must be able to reconstruct which agent acted on whose behalf and on what instruction. Sigstore and in-toto attestation are the analogous patterns from software supply-chain security \citep{intoto-spec,sigstore-2022}; expect agent-attestation standards to converge there.

\textbf{Liability across the boundary.} This is contractual, not technical. The A2A contract has to specify (i)~the GDPR role split for data crossing the boundary --- controller, processor, or joint controllers (Articles~4,~26,~28) \citep{gdpr-2016}, (ii)~audit-log access rights, and (iii)~indemnification when one agent's instruction caused harm via another. The EU AI Act's provider/deployer split is the operative axis going forward \citep{eu-ai-act-2024}: the provider of a general-purpose agent and the deployer integrating it carry different obligations, and the A2A edge is where those obligations meet.

Every A2A edge is a contract, a credential, a log, and a kill-switch. The protocol gives you none of these. Build them, name an owner for each, and treat the edge as production infrastructure.
\end{TNnew}
...
\item Every A2A edge is a new trust boundary needing four things the protocol does not give you: delegated authority, federated identity, verifiable provenance, and liability allocation.
...
\item If you have more than one agent in production, document each A2A edge as a contract, a credential, a log, and a kill-switch; name an owner for each.
