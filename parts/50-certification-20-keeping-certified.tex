...
\begin{tikzpicture}[
  ...
  stage/.style={draw=TNNavy, line width=0.6pt, rounded corners=2pt,
    fill=TNNavyTint, text width=2.3cm, align=center, inner sep=6pt,
    minimum height=1.2cm, font=\sffamily\bfseries\color{TNNavy}},
  arr/.style={-{Stealth}, TNTealDeep, line width=0.7pt},
  arrlabel/.style={font=\sffamily\footnotesize\itshape\color{TNBodyText}, align=center}
]
  \node[stage] (a) at (0,0)     {Develop};
  \node[stage] (b) at (2.9,0)   {Pre-cert};
  \node[stage] (c) at (5.8,0)   {Certify};
  \node[stage] (d) at (8.7,0)   {Operate};
  \node[stage] (e) at (11.6,0)  {Trigger};
  \node[stage] (f) at (14.5,0)  {Re-certify};
  \foreach \x/\y/\lbl in {a/b/{evidence}, b/c/{review}, c/d/{verdict},
                          d/e/{incident or drift}, e/f/{reassessment}}
    \draw[arr] (\x) -- node[arrlabel, above]{\lbl} (\y);
  \draw[arr, dashed] (f.south) .. controls +(0,-1) and +(0,-1) .. node[arrlabel, below]{back to operate}  (d.south);
\end{tikzpicture}%
}
