...
\section{The certification decision rubric}

The eight pre-cert artifacts from \autoref{ch:50-certification-18-pre-cert} tell the team \textit{what} evidence to put in the binder. They do not tell the review board \textit{how} to decide what to do with it. The rubric below is the bridge --- the structured judgment that turns a stack of artifacts into one of the three outcomes the previous section named: approved, approved with conditions, or denied.

The rubric is mechanical at the artifact level. For each of the eight artifacts, the board records one of three states --- \textit{present and complete}, \textit{deficient}, or \textit{missing}. The mapping to outcomes follows from there. A binder where all eight are present and complete is an approved binder. A binder with one or more deficient artifacts is a candidate for approved with conditions, provided each deficiency comes with a remediation plan and a deadline the board accepts. A missing artifact is an automatic denial; so is a deficient artifact whose owner cannot produce a credible plan to close the gap. The discipline is to write the state of each artifact down before the vote, not after.

\stoptwocol
\begin{center}
\captionof{table}{The certification decision rubric --- the eight pre-cert artifacts and what each state looks like at the review board.}
\small
\begin{tabular}{@{}p{3.4cm}p{5.6cm}p{5.6cm}@{}}
\toprule
\textbf{Artifact} & \textbf{Present and complete} & \textbf{Deficient} \\
\midrule
...
\begin{tikzpicture}[
  ...
  rel/.style={font=\sffamily\footnotesize\itshape\color{TNTealDeep}, align=center}
]
  \node[corner] (owner)  at (-5,0) {Owner\\\footnotesize\mdseries product / engineering};
  \node[corner] (board)  at ( 5,0) {Review board\\\footnotesize\mdseries risk, legal, security};
  \node[corner] (reg)    at ( 0,-3.5) {Regulator or external\\\footnotesize\mdseries assessor, auditor, registry};

  \draw[-{Stealth}, TNTealDeep, line width=0.7pt] (owner)  -- node[rel, above]{evidence binder} (board);
  \draw[-{Stealth}, TNTealDeep, line width=0.7pt] (board)  -- node[rel, above]{conditions / findings} (owner);
  \draw[-{Stealth}, TNTealDeep, line width=0.7pt] (board)  -- node[rel, right, xshift=4pt]{conformity report} (reg);
  \draw[-{Stealth}, TNTealDeep, line width=0.7pt] (reg)    -- node[rel, left,  xshift=-4pt]{standard updates} (board);
  \draw[-{Stealth}, TNTealDeep, line width=0.7pt] (owner)  -- node[rel, left,  xshift=-4pt]{incidents, disclosures} (reg);
  \draw[-{Stealth}, TNTealDeep, line width=0.7pt] (reg)    -- node[rel, right, xshift=4pt]{registry obligations} (owner);
\end{tikzpicture}%
}
